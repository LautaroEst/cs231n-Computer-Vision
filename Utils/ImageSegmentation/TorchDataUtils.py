...
def SGDTrainModel(model, data, epochs=1, learning_rate=1e-2, sample_loss_every=100, check_on_train=False):
    ...
    try:
        ...
        for e in range(epochs):
            for t, (x,y) in enumerate(train_dataloader):
                ...
                if (e * batch_size + t) % sample_loss_every == 0:
                    ...
                    print('Accuracy on validation dataset: {}/{} '.format(num_correct_val, num_samples_val))
                    
                    if check_on_train:
                        num_correct_train, num_samples_train = CheckAccuracy(train_dataloader, model, device, input_dtype, target_dtype)
                        print('Accuracy on train dataset: {}/{} '.format(num_correct_train, num_samples_train))
                        print()
                    else:
                        print()
        ...
        num_correct_val, num_samples_val = CheckAccuracy(val_dataloader, model, device, input_dtype, target_dtype)
        print('Final accuracy on validation dataset: {:.2f}%'.format(100 * float(num_correct_val) / num_samples_val) )
        if check_on_train:
            num_correct_train, num_samples_train = CheckAccuracy(train_dataloader, model, device, input_dtype, target_dtype)
            print('Final accuracy on train dataset: {}/{} '.format(num_correct_train, num_samples_train))
